...
void Problem::assembleGlobalSystem(Sparse::Matrix &A, Sparse::Vector &rhs)
{
	...
	for(Mesh::iteratorFace iface = m.BeginFace(); iface != m.EndFace(); iface++){
		...
		if(f.Boundary()){
			...
			if(BCtype == BC_NEUM){
				...
				rMatrix dA = xf - xA;
				double nrm = dA.FrobeniusNorm();
				nrm *= nrm;
				// double dA[2] = {xf[0] - xA[0], xf[1] - xA[1]};
				t  = f.Area() * (nf.Transpose() * D * dA)(0, 0) / nrm;
				// t  = (Dx * dA[0] + Dxy * dA[1]) * nf.data()[0] +
				// (Dxy * dA[0] + Dy * dA[1]) * nf.data()[1];
				// t /= dA[0] * dA[0] + dA[1] * dA[1];

				int indA = cA.Integer(tagGlobInd);
				A[indA][indA] -= t;
				rhs[cA.Integer(tagGlobInd)] -= t * f.Real(tagBCval);
		...
		else
		{
			...
			rMatrix D1(2, 2);
			D1(0, 0) = cA.RealArray(tagD)[0];
			D1(0, 1) = D1(1, 0) = cA.RealArray(tagD)[2];
			D1(1, 1) = cA.RealArray(tagD)[1];

			rMatrix D2(2, 2);
			D2(0, 0) = cB.RealArray(tagD)[0];
			D2(0, 1) = D2(1, 0) = cB.RealArray(tagD)[2];
			D2(1, 1) = cB.RealArray(tagD)[1];

			rMatrix dA = xf - xA;
			double nrmA = dA.FrobeniusNorm();
		        nrmA *= nrmA;	
			rMatrix dB = xf - xB;
			double nrmB = dB.FrobeniusNorm();
		        nrmB *= nrmB;

			double v1  = (nf.Transpose() * D1 * dA)(0, 0) / nrmA;
			double v2  = (nf.Transpose() * D2 * dB)(0, 0) / nrmB;

			double t = f.Area() * (v1 * v2) / (v1 - v2);
	...
	for(auto icell = m.BeginCell(); icell != m.EndCell(); icell++){
		...
		double center[2];
		c.Barycenter(center);
		rhs[c.Integer(tagGlobInd)] += source(center[0], center[1]) * c.Volume();
	}
